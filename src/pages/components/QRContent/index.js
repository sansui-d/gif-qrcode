...
function QRContent(props) {
    ...
    useEffect(() => {
        ...
        cloneSvg.getElementsByTagName('svg')[0].removeChild(cloneSvg.getElementsByTagName('image')[0])
        domtoimage.toSvg(cloneSvg, { bgcolor: '#fff' })
    ...
    return (<div className='gif-qrcode-content-qrcontent'>
        <div className='gif-qrcode-content-qrcontent-container'>
            <div id='gif-qrcode-content-sketch'>
                <QRImage value={val} {...parameter} className='gif-qrcode-content-qrcontent-qr' />
            </div>
            {qr && <img className='gif-qrcode-content-clone-img' src={qr} />}
        </div>
        <Input value={val} onChange={setVal} placeholder='https://sansui-d.github.io/gif-qrcode' />
    </div>);
}
